<cfoutput>
  <h3>#variables.shortdesc#
    <cfif isdefined("url.debug")>
      |#variables.allday#|#variables.enddateno#||#variables.enddate#|#variables.startdate#
    </cfif>
  </h3>
  <p class="time">
    <cfif (isDefined("variables.allday") AND variables.allday EQ 1)>
      #dateformat(variables.startdate,"dddd, mmmm d, yyyy")#
      <cfif enddate is not '' AND datecompare(variables.startdate,variables.enddate,"d") NEQ 0>
        to #dateformat(variables.enddate,"mmmm d, yyyy")#
      </cfif>
      <cfelseif variables.enddateno IS 1 OR variables.enddate eq ''>
      #dateformat(variables.startdate,"mmmm d, yyyy")# #timeformat(variables.startdate,"h:mm TT")#
      <cfelseif datecompare(variables.startdate,variables.enddate,"d") NEQ 0>
      #dateformat(variables.startdate,"mmmm d, yyyy")# at #timeformat(variables.startdate,"h:mm TT")# - #dateformat(variables.enddate,"mmmm d, yyyy")# at #timeformat(variables.enddate,"h:mm TT")#
      <cfelseif datecompare(variables.startdate,variables.enddate,"m") EQ 0 >
      #dateformat(variables.startdate,"mmmm d, yyyy,")# from #timeformat(variables.startdate,"h:mm TT")# - #timeformat(variables.enddate,"h:mm tt")#
      <cfelse>
      huh<!---	--->
    </cfif>
  </p>
  <dl>
    <cfif (variables.information neq ''  and variables.information neq 21)>
      <dt>Description</dt>
      <dd>
        <cfif information is not '' and information is not 'x'>
          <p class="info">#variables.Information#</p>
        </cfif>
      </dd>
    </cfif>
    ...
    <cfif contact is not ''>
      <dt>Contact</dt>
      <dd>#variables.contact#
        <cfif isValid("email", variables.contactinfo)>
          <a href="mailto:#variables.contactinfo#">#variables.contactinfo#</a>
          <cfelse>
